...
impl Parser {
    ...
    fn parse_config(file_data: &str) -> Result<Config, String> {
        ...
        let head_start = {
            ...
            index.parse().expect(&format!("Invalid head start index provided (\"{index}\"). It must be a non negative integer."))
        };
                    ...
                    if current_state.is_some() {
                        ...
                        if state.is_initial {
                            if initial_state_name.is_some() {
                                return Err(format!(
                                    "There was more than one initial state provided."
                                ));
                            }
